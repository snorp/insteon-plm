...
export class Modem extends EventEmitter2 {
   ...
   */
  async startAllLinking({ controller = false, group = 1 } = {}) {
    await this.send({ command: Commands.START_ALL_LINKING, payload: { controller, group } });
  }
  ...
  async getAllLinkDatabase() {
    ...
    await this.send({ command: Commands.GET_FIRST_ALL_LINK });
    ...
    while (true) {
      ...
      const frame = await this.matchFrame(Commands.ALL_LINK_RECORD_RESPONSE);

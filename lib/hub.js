...
export class Hub extends EventEmitter2 {
  ...
  async status(address,  { timeout = DEFAULT_TIMEOUT_MS } = {}) {
    checkAddress(address);
    ow(timeout, ow.number.greaterThan(0));

    const { cmd2 } = await this.messenger.send({
      to: address,
  ...
  async link(address = null, { controller = false, group = 1, timeout = DEFAULT_TIMEOUT_MS } = {}) {
